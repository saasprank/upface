        ...
        <MotionReveal delay={0.1}>
          <div className="rounded-2xl overflow-hidden glass-card">
              ...
              {stats.map((s) => (
                <div key={s.key} className="bg-white p-3 text-center">
                  ...
                  <p className="text-sm font-black" style={{ fontFamily: 'Satoshi, sans-serif', color: s.color }}>
                    {s.value}
                    <span className="text-[10px] font-normal text-faint">{s.suffix}</span>
                  </p>
                </div>
              ))}
            </div>
            ...
            <div className="p-4 border-b border-[rgba(59,130,246,0.08)]">
              <p className="text-[10px] tracking-wider uppercase text-faint mb-3">{t('chart_title')}</p>
              <svg viewBox={`0 0 ${w} ${h + 10}`} className="w-full h-20">
                ...
                <path d={`${pathD} L ${w} ${h + 10} L 0 ${h + 10} Z`} fill="url(#chartFill)" />
                <motion.path
                  d={pathD}
                  fill="none"
                  stroke="#3B82F6"
            ...
            <div className="p-4">
              <p className="text-[10px] tracking-wider uppercase text-faint mb-3">{t('routine_title')}</p>
                ...
                {routineItems.map((item, i) => (
                  ...
                  >
                    <div
                      ...
                      style={{
                        borderColor: i < 2 ? 'rgba(16,185,129,0.45)' : 'rgba(59,130,246,0.25)',
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-xs font-medium text-theme truncate">{t(`${item}_title`)}</p>
                      <p className="text-[10px] text-faint">{t(`${item}_time`)}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              {(['home', 'analyse', 'routine', 'coach', 'profil'] as const).map((tab, i) => (
                <div key={tab} className="flex flex-col items-center gap-0.5">
                  <div
                    ...
                    style={{
                      background: i === 0 ? 'rgba(59,130,246,0.12)' : 'rgba(15,23,42,0.04)',
                      border: i === 0 ? '1px solid rgba(59,130,246,0.25)' : '1px solid transparent',
                    }}
                  />
                  <span className={`text-[8px] ${i === 0 ? 'text-cyan' : 'text-faint'}`}>{t(`nav_${tab}`)}</span>
                </div>
              ))}
            </div>
          </div>
