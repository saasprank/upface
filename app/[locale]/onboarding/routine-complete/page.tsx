...
function RoutineCompleteContent() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('upface_routine')
      if (raw) setRoutine(JSON.parse(raw) as GeneratedRoutine)
    } catch { /* ignore */ }
  }, [])
    ...
    <div className="min-h-screen pb-32" style={{ background: '#080C14' }}>
      ...
      <div className="px-4 pt-8 max-w-lg mx-auto">
        ...
        {categories.length > 0 ? (
          ...
          <div className="text-center py-12" style={{ color: '#5C6B85' }}>
            <p className="text-sm">Charge ta routine depuis le dashboard.</p>
          </div>
        )}
      </div>
